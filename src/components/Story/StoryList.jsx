...
export default function StoryList({ id }) {
  ...
  return (
    <Box id="btn-wrapper" /*onPointerDown={onPointerEvent}*/ >
      {/* <input type="button" value="<" class="leftBtn" /> */}
      <Box p="2em" pl="2em" minHeight="10em" id="wrapper" onMouseDown={handleOnMouse} onMouseUp={handleOnMouse} onMouseMove={handleOnMouse} onMouseLeave={handleOnMouse} >
        {filterId.map((element, index) => (
          <StoryItem key={index} item={element} />
        ))}
      </Box>
      {/* <input type="button" value=">" class="rightBtn" /> */}
    </Box>
  )
}
